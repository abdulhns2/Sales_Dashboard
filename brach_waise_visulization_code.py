...
unique_customers = df_filtered["Customer Name"].nunique()
discounted_sales = df_filtered[df_filtered["Is Discounted"] == 1]["Sales"].sum()
discount_pct = (discounted_sales / total_sales * 100) if total_sales > 0 else 0
...
with col6:
    st.markdown(f"<div class='kpi-card'><div class='kpi-title'>👥 Unique Customers</div><div class='kpi-value'>{unique_customers:,}</div></div>", unsafe_allow_html=True)
with col7:
    st.markdown(f"<div class='kpi-card'><div class='kpi-title'>🎟️ Discount %</div><div class='kpi-value'>{discount_pct:.2f}%</div></div>", unsafe_allow_html=True)
...
def generate_report_html():
    ...
    <body>
        ...
        <div class="kpi"><strong>Unique Customers:</strong> {unique_customers}</div>
        <div class="kpi"><strong>Discount Sales:</strong> {discounted_sales:,.0f}</div>
        <div class="kpi"><strong>Discount %:</strong> {discount_pct:.2f}%</div>

        <h2>Branch Sales</h2>
        {branch_sales.to_html(index=False)}

        <h2>Top Products by Sales</h2>
        {top_products_sales.to_html(index=False)}
...
# discounted_sales = df_filtered[df_filtered["Is Discounted"] == 1]["Sales"].sum()
# discount_pct = (discounted_sales / total_sales * 100) if total_sales > 0 else 0
# annual_sales = df[df["Year"] == datetime.now().year]["Sales"].sum()
...
#     st.markdown(f"<div class='kpi-card'><div class='kpi-title'>🎟️ Discount %</div><div class='kpi-value'>{discount_pct:.2f}%</div></div>", unsafe_allow_html=True)
# with col8:
#     st.markdown(f"<div class='kpi-card'><div class='kpi-title'>📅 Annual Sales ({datetime.now().year})</div><div class='kpi-value'>{annual_sales:,.0f}</div></div>", unsafe_allow_html=True)
...
# fig = px.line(hour_sales, x="Hour", y="Sales", markers=True, title="Hourly Sales Trend")
# st.plotly_chart(fig, use_container_width=True)
...
# st.subheader("📊 Annual Sales Trend (Year-over-Year)")
# annual_trend = df.groupby("Year")["Sales"].sum().reset_index()
# fig = px.bar(annual_trend, x="Year", y="Sales", text="Sales", title="Annual Sales Trend")
...
# # =======================
# branch_sales = df_filtered.groupby("Shop")["Sales"].sum().reset_index()
# top_products_sales = df_filtered.groupby("Product Name")["Sales"].sum().reset_index().nlargest(10, "Sales")
# payment_summary = df_filtered.groupby("MOP")["Sales"].sum().reset_index()
...
#         <div class="kpi"><strong>Discount Sales:</strong> {discounted_sales:,.0f}</div>
#         <div class="kpi"><strong>Discount %:</strong> {discount_pct:.2f}%</div>
#         <div class="kpi"><strong>Annual Sales:</strong> {annual_sales:,.0f}</div>
